...
import { invalidateGitStatus } from "./git-status.mjs";
...
export function startGitFeed(project, debounceMs, emit, opts = {}) {
  ...
  function check() {
    // v3.3.2 — `moved` drives EVENT-DRIVEN CACHE INVALIDATION (lib/git-status.mjs holds a
    // stale-while-revalidate snapshot per repo, so without this a commit would not appear on the
    // board until the TTL elapsed). It is set ONLY on a semantic change (HEAD or the tag list
    // actually moved), never on raw .git filesystem activity — `git status` itself writes
    // .git/index to refresh its stat cache, so invalidating on every fs event would make the
    // status scan retrigger itself forever. That feedback shape is precisely the class of bug this
    // release exists to remove, so it must not be reintroduced here.
    let moved = false;
    ...
    lastTags = tags;
    if (moved) invalidateGitStatus(cwd);
  }
