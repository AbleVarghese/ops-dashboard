...
export async function buildBoardState(project, config) {
  ...
  return {
    ...
    // v3.3.3 — an empty agent list has two very different meanings and the board used to render
    // them identically: "this project genuinely has no agents running" and "I derived a transcript
    // directory that does not exist, so I looked nowhere". The second masqueraded as the first for
    // every project whose path contains a dot — ~/.claude included — until 2026-09-09. Callers MUST
    // distinguish them: an empty list with transcriptSource.ok === false is UNKNOWN, never zero.
    transcriptSource: {
      ok: project.claudeProjectDirExists,
      dir: project.claudeProjectDir,
      detail: project.claudeProjectDirExists
        ? null
        : `No transcript directory at ${project.claudeProjectDir}. The agent list below is UNKNOWN, not empty — ` +
          `this project's directory name was derived from its repo path, and the derived name does not exist. ` +
          `Check that the repoPath is right, or that Claude Code has ever run in it.`,
    },
    agents,
