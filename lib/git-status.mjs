...
// ── PER-REPOSITORY SINGLE-FLIGHT + SHORT-TTL CACHE ───────────────────────────────────────────
...
// every refresh would still queue ~10+2R+3B redundant git calls behind the semaphore and the
// dashboard's latency would grow without bound under a feed-event burst.
//
// Two states, one map:
//   in flight  → every later caller receives the SAME promise (single-flight join). There can
//                never be two concurrent snapshots of one repo, no matter how many timers,
//                watchers, SSE connects and HTTP requests fire at once.
//   settled    → reused for TTL_MS, then recomputed on the next request.
// A snapshot that was DEGRADED (a git call timed out or git could not be spawned) is never cached:
// it is dropped the moment it settles so the very next request retries for real. That is the
// difference between a transient hiccup and a dashboard frozen on stale data for a TTL window.
const snapshots = new Map(); // repoPath -> { promise, settledAt|null, degraded }

function ttlMs() {
  const raw = Number(process.env.OPS_DASH_GIT_TTL_MS);
  // 2000ms: comfortably longer than the 150ms feed-event debounce (so a burst of watcher events
  // collapses onto one snapshot) and comfortably shorter than the 5000ms backstop refresh (so the
  // periodic tick always sees genuinely fresh data and the board never looks frozen).
  return Number.isFinite(raw) && raw >= 0 ? raw : 2000;
}

/** Test/ops seam — drops every cached snapshot. Production never needs this; the TTL handles it. */
...
export async function getGitStatus(project) {
  ...
  if (existing) {
    if (existing.settledAt === null) return existing.promise; // single-flight join
    if (!existing.degraded && Date.now() - existing.settledAt < ttlMs()) return existing.promise; // cache hit
  }

  const rec = { promise: null, settledAt: null, degraded: false };
  rec.promise = (async () => {
    try {
      ...
      // Both failure shapes are evicted immediately rather than left to expire: a thrown snapshot
      // and a degraded one are equally unfit to serve a second consumer, and leaving either in the
      // map is exactly the "permanently poisoned state" this design has to rule out.
      if (rec.degraded) snapshots.delete(key);
    }
  })();
  rec.promise.catch(() => {
    if (snapshots.get(key) === rec) snapshots.delete(key);
  });
  snapshots.set(key, rec);
  return rec.promise;
}
